...
function getErrorDetail(err: unknown): unknown {
  if (typeof err === 'object' && err !== null) {
    ...
    if (maybe.response && 'data' in maybe.response) return maybe.response.data;
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (!API_KEY) {
    return res
      .status(500)
      .json({ error: 'Missing NEXT_PUBLIC_GEMINI_API_KEY' });
  }
  if (req.method !== 'POST') {
  ...
  try {
    ...
    return res.status(200).json({ text, toolCall });
  } catch (err: unknown) {
    const detail = getErrorDetail(err);
    console.error('Agent error', detail);
    return res
      .status(503)
      .json({ error: 'Agent temporarily unavailable', detail });
  }
}
